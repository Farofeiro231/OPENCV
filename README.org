...
* Labeling
This folder contains a simple program to detect and count the number of bubbles (or contiguous regions distinct from the background) in a figure. To observe its results (which are divided in three different output images) I used the same bubbles image as the one in the previous section. Here's how the program works:
